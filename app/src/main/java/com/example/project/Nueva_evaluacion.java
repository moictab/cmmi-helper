...
public class Nueva_evaluacion extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Cursor cursor = database.rawQuery("SELECT * FROM Organizaciones WHERE nombre_org = '"+ nombre_org +"'", null);
		cursor.moveToFirst();
		
		notas_org = cursor.getString(2);
		cursor.close();

		final EditText notasOrganizacion = (EditText) findViewById(R.id.notasOrganizacion);
		notasOrganizacion.setKeyListener(null);
		notasOrganizacion.setFocusable(false);
		
		notasOrganizacion.setText(notas_org);
		
		final EditText editTextNombre = (EditText) findViewById (R.id.nombreOrganizacion);
		editTextNombre.setText(nombre_org);
		editTextNombre.setKeyListener(null);
		editTextNombre.setFocusable(false);
		
		TextView introduccionEtapas = (TextView) findViewById (R.id.textView1);
		TextView introduccionContinua = (TextView) findViewById (R.id.textView2);
		...
		Button botonAyudaCMMI = (Button) findViewById(R.id.button1);
		...
		Button continua = (Button) findViewById (R.id.button3);
		...
		Button etapas = (Button) findViewById (R.id.button2);
