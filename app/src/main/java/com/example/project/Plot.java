...
public class Plot extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState)
    {
        ...
        SharedPreferences preferenciasCM = getSharedPreferences("gestion_configuracion_"+String.valueOf(indiceEvaluacion),Context.MODE_PRIVATE);	
        int cm = preferenciasCM.getInt("nivel", 0);
        
        SharedPreferences preferenciasMA = getSharedPreferences("medicion_analisis_"+String.valueOf(indiceEvaluacion),Context.MODE_PRIVATE);	
        int ma = preferenciasMA.getInt("nivel", 0);
        
        SharedPreferences preferenciasPMC = getSharedPreferences("monitorizacion_control_proyecto_"+String.valueOf(indiceEvaluacion),Context.MODE_PRIVATE);	
        int pmc = preferenciasPMC.getInt("nivel", 0);
        
        SharedPreferences preferenciasPP = getSharedPreferences("planificacion_proyecto_"+String.valueOf(indiceEvaluacion),Context.MODE_PRIVATE);	
        int pp = preferenciasPP.getInt("nivel", 0);
        
        SharedPreferences preferenciasPPQA = getSharedPreferences("aseguramiento_calidad_proceso_producto_"+String.valueOf(indiceEvaluacion),Context.MODE_PRIVATE);	
        int ppqa = preferenciasPPQA.getInt("nivel", 0);
        
        SharedPreferences preferenciasREQM = getSharedPreferences("gestion_requisitos_"+String.valueOf(indiceEvaluacion),Context.MODE_PRIVATE);	
        int reqm = preferenciasREQM.getInt("nivel", 0);
        
        SharedPreferences preferenciasSAM = getSharedPreferences("gestion_acuerdos_proveedores_"+String.valueOf(indiceEvaluacion),Context.MODE_PRIVATE);	
		...
		drawChart(5, cm*base+5, ma*base+5, pmc*base+5, pp*base+5, ppqa*base+5, reqm*base+5, sam*base+5);
		
		Button botonContinuar = (Button) findViewById (R.id.button2);
    ...
    EditText editNotas = (EditText) findViewById (R.id.editText2);
    
	SQLiteDatabase database = gestor.getWritableDatabase();
	Cursor cursor = database.rawQuery("SELECT * FROM Evaluaciones WHERE cod_ev = '" + indiceEvaluacion + "'", null);
	boolean bol = cursor.moveToFirst();
	
	String nombre = cursor.getString(1).toString();
	String notas = cursor.getString(3).toString();
